...
#         self.assertEqual(resp.status_code, 200)
    ...
    def test_product_update_view_url_post(self):
        self.client = Client()
        self.user3 = User.objects.create_user('john3', 'lennon@thebeatles.com', 'johnpassword')
        self.client.login(username='john3', password='johnpassword')
        self.permission1 = Permission.objects.get(name='Can change product')
        self.user3.user_permissions.add(self.permission1)
        self.product1 = Product.objects.create(
            category=Category.objects.create(name='Apple2', slug='apple2'),
            seller=Seller.objects.create(user=self.user3, name='john3'),
            title='iPhone 20',
            slug='iphone-22',
            description="Heeeeeey",
            price=23.00,
            is_active=True,
            views=1,)
        resp = self.client.post(reverse('product_update', kwargs={'pk': self.product1.id}), {
            'category': 'Apple2',
            'seller': 'john2',
            'title': 'iPhone 220',
            'slug': 'iphone-22',
        ...
        self.assertEqual(self.product1.title, 'iPhone 220')
